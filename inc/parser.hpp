...
private:
	...
	void _token_recognizer(std::string s, int lvl, ServerConf srv);
	std::vector<std::string> _split(std::string s, char c);
	std::string _serverNameCheck(std::string s);
	int _portCheck(std::string s);
	std::vector<int> _hostCheck(std::string s);
	std::string _rootCheck(std::string s);
	int _cmbzCheck(std::string s);
